...
export const exercises: Exercise[] = [
  ...
  },
  {
    ...
    chapterId: "ch10",
    type: "quiz",
    prompt: "「ゾンビプロセス」とは何ですか?",
    choices: [
      "実行は終了しているが、親プロセスが終了ステータスを回収しておらず、プロセス表に残り続けているプロセス",
      "無限ループに陥って動き続けているプロセス",
      "他のユーザーのプロセスを不正に乗っ取ったプロセス",
      "バックグラウンドで正常に動作し続けているプロセス",
    ],
    correctChoiceIndex: 0,
    hints: ["実体としての処理はすでに終わっているのがポイントです。"],
    explanation:
      "ゾンビプロセスとは、実行自体はすでに終了しているものの、親プロセスが終了ステータスを回収(wait)しておらず、プロセス表にその情報が残り続けている状態のプロセスです。",
  },
  {
    id: "ch10-ex123",
    chapterId: "ch10",
    type: "quiz",
    prompt: "「孤児プロセス」とは何ですか?また、孤児プロセスは通常どうなりますか?",
    choices: [
      "親プロセスが先に終了した子プロセスのことで、通常はinit(またはsystemd)に引き取られる",
      "PIDを持たないプロセスのことで、システムから即座に削除される",
      "ネットワーク接続が切断されたプロセスのことで、自動的に再接続される",
      "ファイルを持たないプロセスのことで、必ずエラーになる",
    ],
    correctChoiceIndex: 0,
    hints: ["親が先にいなくなった子プロセスは「孤児」と呼ばれます。"],
    explanation:
      "孤児プロセスとは、親プロセスが子プロセスより先に終了してしまった場合の子プロセスのことです。通常はinit(またはsystemd)などのプロセスに引き取られ、管理が引き継がれます。",
  },
  {
    id: "ch10-ex124",
    chapterId: "ch10",
  ...
  },
  {
    ...
    chapterId: "ch10",
    type: "quiz",
    prompt: "topコマンドとpsコマンドの違いとして、最も適切なものはどれですか?",
    choices: [
      "psは実行時点の1回限りのスナップショットを表示するが、topはリアルタイムで情報を更新し続けながら表示する",
      "psはネットワークの状態を、topはプロセスの状態を表示するというように対象が異なる",
      "psはroot専用、topは一般ユーザー専用のコマンドである",
      "psとtopは全く同じ出力をする、単なる別名同士のコマンドである",
    ],
    correctChoiceIndex: 0,
    hints: ["topは画面が自動的に更新され続けるのが特徴です。"],
    explanation:
      "ps は実行した時点でのプロセス一覧を1回だけ表示する(スナップショット)のに対し、topは画面を一定間隔でリアルタイムに更新し続けながらプロセスの状態を表示します。",
  },
  {
    id: "ch10-ex126",
    chapterId: "ch10",
    type: "quiz",
    prompt: "niceコマンドの役割として、最も適切なものはどれですか?",
    choices: [
      "プロセスの実行優先度(nice値)を指定して起動する",
      "プロセスのファイル名を変更する",
      "プロセスの所有者を変更する",
      "プロセスをバックグラウンドで起動する",
    ],
    correctChoiceIndex: 0,
    hints: ["nice値が大きいほど、CPUの優先度は低くなります。"],
    explanation: "nice は、プロセスの実行優先度(nice値)を指定してコマンドを起動するためのコマンドです。nice値が大きいほど他のプロセスに対して優先度が低くなります。",
  },
  {
    id: "ch10-ex127",
    chapterId: "ch10",
    type: "quiz",
    prompt: "killallやpkillコマンドが、killコマンドと異なる点として最も適切なものはどれですか?",
    choices: [
      "PIDではなく、プロセス名(コマンド名)を指定して該当するプロセスをまとめて終了できる",
      "root以外のユーザーは絶対に使用できない",
      "終了させたプロセスを後から復元できる",
      "ファイルシステム上のファイルも同時に削除する",
    ],
    correctChoiceIndex: 0,
    hints: ["killallの「all」は、同名の全プロセスを対象にできることに由来します。"],
    explanation: "killallやpkillは、PIDではなくプロセス名(コマンド名)を指定して、該当する全てのプロセスをまとめて終了させられる点がkillと異なります。",
  },
  {
    id: "ch10-ex128",
    chapterId: "ch10",
