...
export const exercises: Exercise[] = [
  ...
  // --- Ch2-3: 内部コマンドと外部コマンド・シェルの終了 ---
  {
    id: "ch02-03-ex31",
    chapterId: "ch02-03",
    type: "quiz",
    prompt: "「内部コマンド(シェルビルトインコマンド)」の説明として、最も適切なものはどれですか?",
    choices: [
      "シェル自身に組み込まれており、新しく別のプロセスを起動せずにシェルが直接処理するコマンド",
      "/bin等のディレクトリに実行ファイルとして存在し、実行のたびに新しいプロセスを起動するコマンド",
      "ネットワーク経由でしか実行できない特殊なコマンド",
      "rootユーザーだけが実行できるコマンド",
    ],
    correctChoiceIndex: 0,
    hints: ["「内部」=シェルの内側に組み込まれている、という意味です。"],
    explanation:
      "内部コマンド(シェルビルトイン)は、シェル自身が直接処理するコマンドで、実行のために新しいプロセスを起動する必要がありません。" +
      "cd・pwd・echo・exit・historyなどが代表例です。",
  },
  {
    id: "ch02-03-ex32",
    chapterId: "ch02-03",
    type: "quiz",
    prompt: "「外部コマンド」の説明として、最も適切なものはどれですか?",
    choices: [
      "/bin や /usr/bin 等のディレクトリに実行ファイルとして存在し、実行時に新しいプロセスを起動して動作するコマンド",
      "シェルのソースコードの一部として組み込まれているコマンド",
      "インターネットに接続していないと存在自体が確認できないコマンド",
      "常にrootユーザーの権限で実行されるコマンド",
    ],
    correctChoiceIndex: 0,
    hints: ["ls や cat のような多くのコマンドは、実体がディスク上のファイルとして存在します。"],
    explanation:
      "外部コマンドはPATH上のディレクトリ(/bin, /usr/bin等)に実行ファイルとして存在するコマンドで、実行するたびに新しいプロセスが起動されます。" +
      "内部コマンドとは異なり、シェル自体には組み込まれていません。",
  },
  {
    id: "ch02-03-ex33",
    chapterId: "ch02-03",
    type: "quiz",
    prompt: "あるコマンド名が内部コマンドか外部コマンドかを調べるために使うコマンドはどれですか?",
    choices: ["type", "find", "chmod", "grep"],
    correctChoiceIndex: 0,
    hints: ["「このコマンドの種類(type)は何?」とそのまま尋ねるようなコマンド名です。"],
    explanation:
      "type コマンドに調べたいコマンド名を引数として渡すと、それが内部コマンドか、外部コマンド(その場合は実行ファイルのパスも)か、エイリアスかなどを教えてくれます。",
  },
  {
    id: "ch02-03-ex34",
    chapterId: "ch02-03",
    type: "quiz",
    prompt: "cd(カレントディレクトリの変更)が外部コマンドではなく内部コマンドとして実装されている理由として、最も適切なものはどれですか?",
    choices: [
      "カレントディレクトリはシェルのプロセス自身が持つ状態であり、別プロセス(外部コマンド)からは変更できないため",
      "cdという名前が3文字と短いため、慣習的に内部コマンドにする決まりがあるため",
      "cdはネットワーク通信を伴うコマンドであるため",
      "外部コマンドは移動(cd)のような操作を一切サポートしていない仕様であるため",
    ],
    correctChoiceIndex: 0,
    hints: ["もしcdが外部コマンド(別プロセス)だったら、そのプロセスの中だけでディレクトリが変わり、呼び出し元のシェルには影響しません。"],
    explanation:
      "カレントディレクトリはシェルのプロセス自身が持つ状態です。" +
      "もしcdが外部コマンド(別の新しいプロセス)として実装されていた場合、そのプロセスの中だけでディレクトリが変わっても、呼び出し元のシェルのカレントディレクトリには影響を与えられません。" +
      "そのためcdは内部コマンドとして実装されています。",
  },
  {
    id: "ch02-03-ex35",
    chapterId: "ch02-03",
  ...
  },
  {
    ...
    chapterId: "ch02-03",
    type: "quiz",
    prompt: "内部コマンドと外部コマンドを比較したとき、一般的な傾向として正しいものはどれですか?",
    choices: [
      "内部コマンドは新しいプロセスを起動しないため、外部コマンドに比べて起動のオーバーヘッドが小さい傾向がある",
      "外部コマンドは常に内部コマンドよりも高速に動作することが保証されている",
      "内部コマンドと外部コマンドとで、処理速度に差が生まれることは理論上あり得ない",
      "内部コマンドは必ずネットワーク経由で実行されるため、外部コマンドより低速である",
    ],
    correctChoiceIndex: 0,
    hints: ["新しいプロセスを起動する処理には、それ自体にコスト(オーバーヘッド)がかかります。"],
    explanation:
      "内部コマンドはシェル自身が直接処理を行い、新しいプロセスを起動する必要がないため、外部コマンドに比べて起動にかかるオーバーヘッドが小さい傾向があります。" +
      "ただし常に外部コマンドより高速というわけではありません。",
  },
  {
    id: "ch02-03-ex38",
    chapterId: "ch02-03",
    type: "quiz",
    prompt:
      "echo のように、内部コマンドとしても外部コマンド(/bin/echo等)としても存在するコマンド名がある場合、何も指定せずにechoと入力すると一般的にどちらが実行されますか?",
    choices: [
      "内部コマンドが優先して実行される",
      "外部コマンドが優先して実行される",
      "両方が同時に実行され、出力が2回表示される",
      "どちらも実行されず、必ずエラーになる",
    ],
    correctChoiceIndex: 0,
    hints: ["シェルはまず自分自身に組み込まれた機能(内部コマンド)を優先的に確認します。"],
    explanation:
      "多くのシェルでは、内部コマンドと同名の外部コマンドが存在する場合、特にパスを指定しなければ内部コマンドが優先して実行されます。" +
      "外部コマンドの方を明示的に実行したい場合は、/bin/echoのようにフルパスを指定します。",
  },
  {
    id: "ch02-03-ex39",
    chapterId: "ch02-03",
    type: "quiz",
    prompt: "type コマンドで調べられる「コマンド名の種類」に含まれないものはどれですか?",
    choices: [
      "そのファイルのパーミッション(rwx等の権限情報)",
      "内部コマンド(シェルビルトイン)であること",
      "外部コマンド(実行ファイルのパス)であること",
      "エイリアスとして定義されていること",
    ],
    correctChoiceIndex: 0,
    hints: ["typeが教えてくれるのは「種類(何者か)」であり、権限の詳細はls -lやstatの役割です。"],
    explanation:
      "typeコマンドは、指定した名前が内部コマンド・外部コマンド(その場合はパス)・エイリアス・関数のいずれであるかを教えてくれます。" +
      "ファイルのパーミッション情報を確認したい場合は、ls -lやstatなど別のコマンドを使います。",
  },
  {
    id: "ch02-03-ex40",
    chapterId: "ch02-03",
