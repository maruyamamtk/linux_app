...
export const exercises: Exercise[] = [
  ...
  },
  {
    ...
    chapterId: "ch01",
    type: "quiz",
    prompt: "VirtualBoxのような、1台の物理PC(ホストOS)上で動くタイプの仮想化ソフトウェアは一般に何と呼ばれますか?",
    choices: ["ホスト型(Type2)ハイパーバイザー", "ベアメタル型(Type1)ハイパーバイザー", "コンテナランタイム", "ブートローダー"],
    correctChoiceIndex: 0,
    hints: ["ホストOSという「土台となるOS」の上にインストールして使うタイプです。"],
    explanation:
      "ホストOS上にアプリケーションとしてインストールし、その上でゲストOSを動かす方式をホスト型(Type2)" +
      "ハイパーバイザーと呼びます。VirtualBoxやVMware Workstationはこの方式です。対して、物理ハードウェア上に" +
      "直接インストールされるものはベアメタル型(Type1)と呼ばれます。",
  },
  {
    id: "ch01-ex07",
    chapterId: "ch01",
    type: "quiz",
    prompt: "仮想マシンにゲストOS(Linux)をインストールする際、通常インストールメディアとして使われるファイル形式はどれですか?",
    choices: ["ISOイメージファイル", "PDFファイル", "圧縮されたzipファイル", "テキストファイル"],
    correctChoiceIndex: 0,
    hints: ["光学ディスク(CD/DVD)の中身をまるごと1つのファイルにしたような形式です。"],
    explanation:
      "ISOイメージは光学ディスクの内容を1つのファイルにまとめた形式で、LinuxディストリビューションのインストーラーはISO" +
      "イメージとして配布されるのが一般的です。VirtualBoxでは仮想の光学ドライブにこのISOイメージをマウントして" +
      "起動し、インストールを行います。",
  },
  {
    id: "ch01-ex08",
    chapterId: "ch01",
    type: "quiz",
    prompt: "VirtualBoxで仮想ハードディスクを作成する際の「可変サイズ(Dynamically allocated)」の説明として正しいものはどれですか?",
    choices: [
      "指定した上限までの範囲で、実際に使用したデータ量に応じてファイルサイズが徐々に増えていく",
      "作成した時点で指定したサイズ分のディスク領域をホストOS側に必ず確保する",
      "仮想ハードディスクの容量はいつでも自由に無制限に変更できる",
      "可変サイズを選ぶと、ゲストOSからは常に無制限の容量として認識される",
    ],
    correctChoiceIndex: 0,
    hints: ["「可変」という名前の通り、最初は小さく、使うにつれて大きくなっていくイメージです。"],
    explanation:
      "可変サイズ(Dynamically allocated)の仮想ハードディスクは、作成直後はごくわずかな実サイズしか消費せず、" +
      "ゲストOS側でデータが増えるにつれてホストOS上のファイルサイズも指定した上限まで徐々に増加していきます。" +
      "対して固定サイズ(Fixed size)は、作成時点で指定した容量分をあらかじめ確保します。",
  },
  {
    id: "ch01-ex09",
    chapterId: "ch01",
    type: "quiz",
    prompt: "VirtualBoxの「Guest Additions」をゲストOSにインストールする主な目的は何ですか?",
    choices: [
      "画面解像度の自動調整やホストOSとのクリップボード共有・共有フォルダなど、統合機能を有効にするため",
      "ゲストOSのカーネルをホストOSと完全に同じバージョンに置き換えるため",
      "ゲストOS上でホストOSのアプリケーションを直接実行できるようにするため",
      "ゲストOSのライセンス認証を行うため",
    ],
    correctChoiceIndex: 0,
    hints: ["インストールしないと、ウィンドウサイズを変えても画面が自動で追従してくれなかったりします。"],
    explanation:
      "Guest AdditionsはゲストOS内にインストールする追加ドライバ・ツール群で、ウィンドウサイズに合わせた" +
      "画面解像度の自動調整、ホストOSとの間でのクリップボード共有、共有フォルダ機能などを有効にし、" +
      "ホストOS・ゲストOS間の操作性を向上させます。",
  },
  {
    id: "ch01-ex10",
    chapterId: "ch01",
    type: "quiz",
    prompt: "VirtualBoxの「共有フォルダ」機能で実現できることはどれですか?",
    choices: [
      "ホストOS上の特定のフォルダを、ゲストOS内からもファイルシステムの一部として参照・利用できるようにする",
      "複数のゲストOS同士が常に同じディスクイメージを共有して排他ロックなしに書き込みを行う",
      "ゲストOSの画面をそのままホストOSのデスクトップの壁紙にする",
      "ホストOSのネットワーク帯域をゲストOSに完全に明け渡す",
    ],
    correctChoiceIndex: 0,
    hints: ["ホストとゲストの間でファイルをやり取りしたいときに使う機能です(コピー&ペーストではなく)。"],
    explanation:
      "共有フォルダはホストOS上のフォルダをゲストOSにマウントし、ゲストOS内のファイルシステムの一部のように" +
      "扱えるようにする機能です。ホスト・ゲスト間でのファイルの受け渡しに便利で、Guest Additionsのインストールが" +
      "前提となります。",
  },
  {
    id: "ch01-ex11",
    chapterId: "ch01",
    type: "quiz",
    prompt: "VirtualBoxのネットワークアダプター設定「NAT」の特徴として正しいものはどれですか?",
    choices: [
      "ゲストOSはホストOSを経由して外部ネットワークに接続できるが、外部から直接ゲストOSへはアクセスしにくい",
      "ゲストOSがホストOSと同じネットワーク上の独立した1台の機器のように、外部から直接見える",
      "ゲストOS同士だけが通信でき、外部ネットワークには一切接続できない",
      "設定した瞬間にゲストOSのIPアドレスがホストOSと完全に同じになる",
    ],
    correctChoiceIndex: 0,
    hints: ["家庭用ルーターの内側にいる端末が外部と通信する仕組みに似ています。"],
    explanation:
      "NAT(Network Address Translation)モードでは、ゲストOSはホストOSを経由してアドレス変換された形で" +
      "外部ネットワークに接続できます。手軽に外部通信ができる反面、外部の機器からゲストOSへ直接アクセスすることは" +
      "デフォルトではできません(ポートフォワーディング設定が必要)。",
  },
  {
    id: "ch01-ex12",
    chapterId: "ch01",
    type: "quiz",
    prompt: "VirtualBoxのネットワークアダプター設定「ブリッジ接続」の特徴として正しいものはどれですか?",
    choices: [
      "ゲストOSがホストOSと同じ物理LAN上の独立した1台の機器として、他の機器から直接見える",
      "ゲストOSは常にインターネットへの接続が完全に遮断される",
      "ホストOSとゲストOSのメモリ空間が物理的に統合される",
      "ブリッジ接続を使うとGuest Additionsのインストールが不要になる",
    ],
    correctChoiceIndex: 0,
    hints: ["「ブリッジ(橋渡し)」という名前の通り、ゲストOSを物理LANに直接橋渡しするイメージです。"],
    explanation:
      "ブリッジ接続では、ホストOSが接続している物理ネットワークインターフェースにゲストOSを直接橋渡しし、" +
      "ゲストOSがLAN上の独立した1台の機器のように振る舞います。同じLAN内の他の機器からゲストOSへ直接" +
      "アクセスできる点がNATモードとの大きな違いです。",
  },
  {
    id: "ch01-ex13",
    chapterId: "ch01",
    type: "quiz",
    prompt: "VirtualBoxのネットワークアダプター設定「ホストオンリーアダプター」の特徴はどれですか?",
    choices: [
      "ホストOSと、同じホストオンリーネットワーク上のゲストOS同士でのみ通信でき、外部ネットワークとは隔離される",
      "外部のインターネットとの通信のみ可能で、ホストOSとは一切通信できない",
      "複数の物理PCをまたいでゲストOS同士を直接通信させる",
      "USBデバイスをゲストOSに直接接続するための設定である",
    ],
    correctChoiceIndex: 0,
    hints: ["外部から隔離された、ホストとゲストだけの閉じたネットワークをイメージしてください。"],
    explanation:
      "ホストオンリーアダプターは、ホストOSと同じホストオンリーネットワークに属するゲストOS同士のみが通信できる、" +
      "外部から隔離された仮想ネットワークです。外部への接続を遮断したまま、ホストOSとの検証用通信だけを行いたい" +
      "場合などに使われます。",
  },
  {
    id: "ch01-ex14",
    chapterId: "ch01",
    type: "quiz",
    prompt: "仮想マシンに割り当てるメモリ(RAM)量を決める際に、最も注意すべき点はどれですか?",
    choices: [
      "ホストOSが利用する分も含めて考え、物理メモリの上限を超えて複数のVMに割り当てすぎないようにする",
      "常にホストOSの物理メモリと全く同じ量を割り当てなければならない",
      "仮想マシンには物理メモリと無関係に無制限の値を設定できる",
      "メモリ割り当てはゲストOSの動作速度に一切影響しない",
    ],
    correctChoiceIndex: 0,
    hints: ["ホストOS自身もメモリを使って動いていることを忘れないようにしましょう。"],
    explanation:
      "仮想マシンに割り当てたメモリはホストOSの物理メモリから確保されます。ホストOS自身の動作分やほかのVM・" +
      "アプリケーションの分を考慮せずに割り当てすぎると、ホストOS全体の動作が重くなったり不安定になったりする" +
      "ため、物理メモリの総量を意識して設定する必要があります。",
  },
  {
    id: "ch01-ex15",
    chapterId: "ch01",
    type: "quiz",
    prompt: "仮想マシンに割り当てるCPUコア数についての説明として適切なものはどれですか?",
    choices: [
      "ホストOSの物理CPUコア数を上限の目安とし、それを超えて割り当てるとかえって性能が低下することがある",
      "ゲストOSに割り当てるコア数は、ホストOSの物理コア数と無関係に無制限に増やせば増やすほど必ず速くなる",
      "CPUコア数の設定はゲストOSの起動速度以外には一切影響しない",
      "1つの仮想マシンには常に1コアしか割り当てられない",
    ],
    correctChoiceIndex: 0,
    hints: ["物理的に存在するコア数を超えて割り当てると、逆にオーバーヘッドが増えることがあります。"],
    explanation:
      "仮想マシンに割り当てるCPUコア数は、ホストOSの物理コア数を目安に設定します。物理コア数を超えて割り当てると" +
      "ホストOS側でのスケジューリングのオーバーヘッドが増え、かえって性能が低下する場合があります。",
  },
  {
    id: "ch01-ex16",
    chapterId: "ch01",
    type: "quiz",
    prompt: "1つの仮想マシンに対して、スナップショットは何回まで取得できますか?",
    choices: [
      "ディスク容量が許す限り、複数回・繰り返し取得しツリー状に分岐させることができる",
      "1つの仮想マシンにつき1回しか取得できない",
      "スナップショットは仮想マシン作成時にのみ自動で1回取得される",
      "スナップショットはゲストOSをシャットダウンしている間しか取得できない",
    ],
    correctChoiceIndex: 0,
    hints: ["「ここまでは大丈夫」というチェックポイントを、作業の節目ごとに何度も置いていくイメージです。"],
    explanation:
      "スナップショットはディスク容量が許す限り何度でも取得でき、取得順にツリー状の履歴として管理されます。" +
      "ある作業前の状態、別の作業前の状態、というように複数のチェックポイントを残し、必要な時点へ自由に戻れます。",
  },
  {
    id: "ch01-ex17",
    chapterId: "ch01",
    type: "quiz",
    prompt: "VirtualBoxの「クローン」機能でできることはどれですか?",
    choices: [
      "既存の仮想マシンの複製をまるごと新しい仮想マシンとして作成する",
      "ゲストOSの画面を撮影してホストOSに1枚の画像として保存する",
      "ゲストOSのCPU使用率をリアルタイムでグラフ表示する",
      "ホストOSをゲストOSの中に入れ子にしてインストールする",
    ],
    correctChoiceIndex: 0,
    hints: ["同じ設定・同じディスク内容を持つ「もう1台」を作りたいときに使う機能です。"],
    explanation:
      "クローン機能は、既存の仮想マシンの設定やディスク内容を複製し、新しい別の仮想マシンとして作成する機能です。" +
      "同じ環境を複数用意したい場合や、検証用に元の環境を残したまま複製を作りたい場合に使われます。",
  },
  {
    id: "ch01-ex18",
    chapterId: "ch01",
    type: "quiz",
    prompt: "仮想マシンを他のPCへ持ち運びたいとき、VirtualBoxで使われる標準的な書き出し形式(拡張子)はどれですか?",
    choices: ["OVA形式(Open Virtualization Appliance)", "PDF形式", "MP4形式", "ZIP形式のみ(専用形式は存在しない)"],
    correctChoiceIndex: 0,
    hints: ["「アプライアンスのエクスポート」機能で書き出すときに使われる、仮想化ソフト間でも比較的互換性のある形式です。"],
    explanation:
      "VirtualBoxの「アプライアンスのエクスポート」機能を使うと、仮想マシンの設定とディスク内容を1つの" +
      "OVAファイルにまとめて書き出せます。OVAは他のPCや(対応していれば)他の仮想化ソフトへ環境を移す際に" +
      "使われる標準的な形式です。",
  },
  {
    id: "ch01-ex19",
    chapterId: "ch01",
    type: "quiz",
    prompt: "仮想マシンの電源操作のうち、ゲストOS内で「シャットダウンボタンを押す」のと同じようにACPI経由で" +
      "正常なシャットダウン処理を要求する操作はどれですか?",
    choices: [
      "ACPIシャットダウン(通常の電源オフ要求)",
      "電源オフ(Power Off、コンセントを抜くのに相当)",
      "一時停止(Pause)",
      "スナップショットの復元",
    ],
    correctChoiceIndex: 0,
    hints: ["実機のPCで電源ボタンを軽く押したときの挙動に近い操作です。"],
    explanation:
      "ACPIシャットダウンは、実機の電源ボタンを押したときと同様にゲストOSへ正常なシャットダウン処理を要求する" +
      "操作です。ゲストOS側がファイルの保存やプロセスの終了処理を行ってから電源が切れるため、いきなり電源を" +
      "落とす「電源オフ」よりもファイルシステムを壊しにくい安全な方法です。",
  },
  {
    id: "ch01-ex20",
    chapterId: "ch01",
    type: "quiz",
    prompt: "仮想マシンをいきなり「電源オフ(Power Off)」する操作は、実機の何をするのに相当しますか?",
    choices: [
      "電源ケーブルを強制的に引き抜くこと",
      "OSの「再起動」メニューを選ぶこと",
      "スリープモードへ移行させること",
    ...
    correctChoiceIndex: 0,
    hints: ["ゲストOS側に終了処理をする時間を与えない、乱暴な止め方です。"],
    explanation:
      "「電源オフ」はゲストOSに正常な終了処理を行わせず、いきなり電源を落とす操作で、実機で電源ケーブルを" +
      "強制的に引き抜くことに相当します。保存していない作業内容が失われたり、まれにファイルシステムが" +
      "不整合な状態になったりするリスクがあるため、通常はACPIシャットダウンが推奨されます。",
  },
  {
    id: "ch01-ex21",
    chapterId: "ch01",
    type: "quiz",
    prompt: "仮想マシンの「一時停止(Pause)」機能の説明として正しいものはどれですか?",
    choices: [
      "CPUの実行を一時的に止めて動作を凍結させるだけで、ゲストOSは終了しない(再開すれば続きから動く)",
      "ゲストOSを完全にシャットダウンしてからディスクの内容だけを保持する",
      "一時停止すると、次回起動時には必ずOSの再インストールから始まる",
      "一時停止と電源オフは全く同じ処理内容である",
    ],
    correctChoiceIndex: 0,
    hints: ["動画の「一時停止」ボタンのように、状態を保ったまま処理だけを止めるイメージです。"],
    explanation:
      "一時停止(Pause)は、ゲストOSの実行(CPU)を一時的に止めて動作を凍結させる機能です。ゲストOSは終了せず、" +
      "メモリの内容もそのまま保持されるため、再開すれば停止した時点の続きから動作します。",
  },
  {
    id: "ch01-ex22",
    chapterId: "ch01",
    type: "quiz",
    prompt: '仮想マシンの終了方法「保存(Save the machine state)」と「スナップショット」の違いとして正しいものはどれですか?',
    choices: [
      "「保存」は現在の実行状態を1つだけ保持して次回そこから再開するための機能、スナップショットは名前を付けて" +
        "複数の時点を履歴として残せる機能",
      "「保存」とスナップショットは全く同じ機能で、呼び方が違うだけである",
      "「保存」は仮想ハードディスクの中身をまるごと削除する機能である",
      "スナップショットは1つの仮想マシンにつき一度しか使えないが、「保存」は何度でも使える",
    ],
    correctChoiceIndex: 0,
    hints: ["スナップショットは「複数の時点」を残せますが、状態の保存(Save State)は基本的に直近の1状態です。"],
    explanation:
      "「状態を保存して終了(Save the machine state)」は、実行中の状態をディスクに書き出して仮想マシンを終了し、" +
      "次回起動時にその続きから再開できるようにする機能です。スナップショットのように名前を付けて複数の時点を" +
      "履歴として残す機能とは異なり、基本的には直近の1状態を保持するものです。",
  },
  {
    id: "ch01-ex23",
    chapterId: "ch01",
    type: "quiz",
    prompt: "作成した仮想マシンにISOイメージから起動させるための、VirtualBox側の設定として必要なものはどれですか?",
    choices: [
      "仮想の光学ドライブにISOイメージファイルをマウントし、起動順序でその光学ドライブを優先させる",
      "ISOイメージファイルを共有フォルダとして常時マウントしておく",
      "ISOイメージの内容をあらかじめすべて仮想ハードディスクにコピーしておく",
      "特に設定は不要で、ISOファイルをダブルクリックするだけでゲストOSが自動的に起動する",
    ],
    correctChoiceIndex: 0,
    hints: ["実機でCD/DVDから起動する場合の「ブート順序」設定に近い考え方です。"],
    explanation:
      "ISOイメージから仮想マシンを起動するには、仮想マシンの設定でISOイメージを仮想の光学ドライブにマウントし、" +
      "起動順序(ブートオーダー)で光学ドライブを優先させる必要があります。これにより、インストーラーの" +
      "ISOイメージから起動してOSのインストールを開始できます。",
  },
  {
    id: "ch01-ex24",
    chapterId: "ch01",
    type: "quiz",
    prompt: "VirtualBoxの仮想マシン作成時に「OSタイプ」を正しく選択することの主なメリットは何ですか?",
    choices: [
      "選択したOSに適したデフォルト設定(推奨メモリ量・チップセット等)が自動的に提案される",
      "OSタイプを選ぶと、そのOSのインストーラーが自動的にダウンロードされる",
      "OSタイプの選択は表示上のアイコンが変わるだけで、動作には一切影響しない",
      "OSタイプを間違えると、その仮想マシンは二度と起動できなくなる",
    ],
    correctChoiceIndex: 0,
    hints: ["選んだOSに応じて、推奨されるメモリ量やディスクサイズの初期値が変わります。"],
    explanation:
      "仮想マシン作成時にOSタイプ(例: Linux/Ubuntu 64-bit)を選択すると、そのOSに適した推奨メモリ量や" +
      "推奨ディスクサイズ、チップセットなどの初期設定が自動的に提案され、手動での細かい調整の手間を減らせます。",
  },
  {
    id: "ch01-ex25",
    chapterId: "ch01",
    type: "quiz",
    prompt: "Linuxコマンドの学習を仮想マシン上で行う場合、GUI(グラフィカルな画面)よりもCUI(コマンドラインインターフェース)" +
      "中心の環境が向いている理由として適切なものはどれですか?",
    choices: [
      "コマンド操作そのものを学ぶことが目的であり、少ないリソースでも軽快に動作するため学習環境として扱いやすい",
      "CUIではマウスやキーボードでの操作が一切できないため",
      "GUI環境ではコマンドを一切実行できないため",
      "CUI環境では仮想マシンを作成すること自体ができないため",
    ],
    correctChoiceIndex: 0,
    hints: ["この教材で身につけたいのは「コマンドの使い方」そのものです。"],
    explanation:
      "コマンド操作の習得が目的である場合、GUIの各種アプリケーションよりも、コマンド入力に集中できるCUI中心の" +
      "環境の方が学習に適しています。また、CUI環境は消費するメモリ・ディスク容量も少ないため、仮想マシンの" +
      "リソースが限られていても軽快に動作させやすいという利点もあります。",
  },
  {
    id: "ch01-ex26",
    chapterId: "ch01",
    type: "quiz",
    prompt: "仮想マシンの電源を切る、削除する、といった操作を行っても、通常ホストOS側に起きることはどれですか?",
    choices: [
      "ホストOS上のファイルやアプリケーションには影響がなく、そのまま利用を続けられる",
      "ホストOS自体が強制的に再起動される",
      "ホストOSにインストールされている他のアプリケーションもすべて削除される",
      "ホストOSのユーザーアカウントがすべてロックされる",
    ],
    correctChoiceIndex: 0,
    hints: ["仮想マシンはホストOSから見ると「独立した1つのファイル群」として扱われています。"],
    explanation:
      "仮想マシンはホストOS上に置かれた独立したファイル群(仮想ハードディスクファイルや設定ファイル)として" +
      "扱われるため、その仮想マシンの電源を切ったり削除したりしても、ホストOS本体の動作やほかのアプリケーション" +
      "には影響しません。これが、失敗を恐れずに学習できる大きな理由です。",
  },
  {
    id: "ch01-ex27",
    chapterId: "ch01",
    type: "quiz",
    prompt: "VirtualBoxで仮想マシンにUSBデバイス(USBメモリ等)を接続するための機能は何ですか?",
    choices: [
      "USBコントローラーを有効化し、ゲストOSへUSBデバイスを渡す機能",
      "共有フォルダ機能(USBデバイスは共有フォルダとしてのみ扱える)",
      "スナップショット機能(USBデバイスの内容もスナップショットに含まれる)",
      "ホストオンリーアダプター(ネットワーク経由でUSBデバイスをエミュレートする)",
    ],
    correctChoiceIndex: 0,
    hints: ["ネットワークの話ではなく、周辺機器を仮想マシンに「渡す」ための機能です。"],
    explanation:
      "VirtualBoxは仮想マシンの設定でUSBコントローラーを有効にし、接続されたUSBデバイスをホストOSから" +
      "ゲストOSへ渡す(パススルーする)ことができます。これにより、ゲストOS上でホストに挿したUSBメモリなどを" +
      "直接認識・利用できます。",
  },
  {
    id: "ch01-ex28",
    chapterId: "ch01",
    type: "quiz",
    prompt: "1台の物理PC(ホストOS)上で、VirtualBoxを使って複数の仮想マシンを同時に起動することについて正しい説明はどれですか?",
    choices: [
      "ホストOSの物理リソース(CPU/メモリ)が許す範囲であれば、複数の仮想マシンを同時に起動できる",
      "VirtualBoxでは仕組み上、一度に1台の仮想マシンしか起動できない",
      "複数の仮想マシンを同時に起動すると、必ずディスクの内容が破損する",
      "複数起動するには、それぞれ別のホストOSを用意する必要がある",
    ],
    correctChoiceIndex: 0,
    hints: ["制約になるのは主にホストの物理リソース(CPU・メモリ)です。"],
    explanation:
      "VirtualBoxではホストOSの物理CPU・メモリ等のリソースが許す範囲内であれば、複数の仮想マシンを同時に起動して" +
      "並行して利用できます。ただし、それぞれの仮想マシンに割り当てたリソースの合計がホストの実リソースを大きく" +
      "超えると、動作が重くなったり不安定になったりします。",
  },
  {
    id: "ch01-ex29",
    chapterId: "ch01",
    type: "quiz",
    prompt: "VirtualBox以外にも仮想化ソフトウェアは存在しますが、代表的な製品の組み合わせとして適切なものはどれですか?",
    choices: [
      "VMware WorkstationやHyper-Vなど",
      "photoshopやIllustratorなど",
      "ApacheやNginxなど",
      "GitやSubversionなど",
    ],
    correctChoiceIndex: 0,
    hints: ["Adobe製品や画像編集ソフトの話ではありません。仮想マシンを作れるソフトの仲間を選びましょう。"],
    explanation:
      "VirtualBoxのほかにも、VMware WorkstationやMicrosoftのHyper-Vなど、複数の仮想化ソフトウェアが存在します。" +
      "いずれも1台の物理PC上で仮想マシンを作成・実行できる点は共通ですが、対応OSやライセンス形態などに違いが" +
      "あります。",
  },
  {
    id: "ch01-ex30",
    chapterId: "ch01",
    type: "quiz",
    prompt: "AWSなどのクラウド上に用意される仮想サーバーと、手元のPCでVirtualBoxを使って作る仮想マシンの違いとして" +
      "適切なものはどれですか?",
    choices: [
      "クラウド上の仮想サーバーはインターネット越しに提供される他社のデータセンター上のリソースを使うのに対し、" +
        "VirtualBoxの仮想マシンは自分のPCのリソースだけで完結する",
      "クラウド上の仮想サーバーは無料でしか提供されない",
      "VirtualBoxの仮想マシンは常にインターネット接続がないと起動できない",
      "両者は技術的に全く同じものであり、呼び方が違うだけである",
    ],
    correctChoiceIndex: 0,
    hints: ["「どこの物理マシンのリソースを使っているか」に注目しましょう。"],
    explanation:
      "AWSなどのクラウドサービスが提供する仮想サーバーは、事業者が保有するデータセンターの物理マシン上に" +
      "作られ、インターネット経由で利用します。一方、VirtualBoxの仮想マシンは手元のPC(ホストOS)自身の" +
      "リソース上で完結して動作するため、学習用途ではネットワーク接続なしでも利用できます。",
  },
  {
    id: "ch01-ex31",
    chapterId: "ch01",
    type: "quiz",
    prompt: "仮想マシンにゲストOSをインストールする際、実機と同様に必要になる作業はどれですか?",
    choices: [
      "仮想ハードディスク上にパーティションを切り、ファイルシステムを作成する",
      "ホストOSのパーティションを必ず縮小して空き領域を作る",
      "ホストOSのブートローダーを完全に消去する",
      "ホストOS用のライセンスキーをゲストOSにも入力する",
    ],
    correctChoiceIndex: 0,
    hints: ["「まっさらな仮想ハードディスク」に対して行う作業は、実機のディスクへのインストールとほぼ同じです。"],
    explanation:
      "仮想マシンへのOSインストールでも、実機と同様に仮想ハードディスク上にパーティションを作成し、" +
      "ファイルシステムを構築する作業が必要です(インストーラーが自動的に行ってくれることが多いですが、" +
      "手順としては実機のインストールと共通しています)。ホストOS側のディスクには影響しません。",
  },
  {
    id: "ch01-ex32",
    chapterId: "ch01",
    type: "quiz",
    prompt: "仮想マシンの電源を「電源オフ」でいきなり落とすことのリスクとして最も適切なものはどれですか?",
    choices: [
      "書き込み中だったファイルの内容が失われたり、ファイルシステムが不整合な状態になったりする可能性がある",
      "ホストOSのファイルシステムが必ず破損する",
      "次回起動時に仮想ハードディスクの容量が自動的に2倍になる",
      "ネットワークアダプターの設定が自動的にNATからブリッジ接続に変わる",
    ],
    correctChoiceIndex: 0,
    hints: ["ACPIシャットダウンとの違いを思い出しましょう。終了処理を挟むかどうかがポイントです。"],
    explanation:
      "「電源オフ」はゲストOSに正常な終了処理をさせずに電源を落とすため、実機で電源ケーブルを急に抜くのと同様に、" +
      "書き込み中だったファイルの内容が失われたり、まれにファイルシステムが不整合な状態になったりするリスクが" +
      "あります。通常はACPIシャットダウンなど、ゲストOSに正常終了させる方法が推奨されます。",
  },
  {
    id: "ch01-ex33",
    chapterId: "ch01",
    type: "quiz",
    prompt: "「仮想マシンを削除する」操作について、正しい理解はどれですか?",
    choices: [
      "仮想ハードディスクファイルまで含めて削除すると、そのVM内に保存していたデータは失われる",
      "仮想マシンを削除しても、必ず仮想ハードディスクの中身はホストOSのデスクトップに自動的に取り出される",
      "仮想マシンの削除は取り消しのできる操作であり、いつでも元に戻せる",
      "仮想マシンを削除すると、ホストOSにインストールされたVirtualBox自体も同時にアンインストールされる",
    ],
    correctChoiceIndex: 0,
    hints: ["削除は基本的に元に戻せない操作であり、対象がディスクファイルまで含むかどうかが重要です。"],
    explanation:
      "仮想マシンを削除する際、「関連ファイルもすべて削除」を選ぶと仮想ハードディスクファイルごと削除され、" +
      "そのVM内に保存していたデータもすべて失われます。事前にスナップショットを消したり、必要なデータを" +
      "共有フォルダなどでバックアップしたりしておく必要があります。",
  },
  {
    id: "ch01-ex34",
    chapterId: "ch01",
    type: "quiz",
    prompt: "VirtualBoxで仮想マシンの設定を変更できるタイミングとして、一般的に正しいものはどれですか?",
    choices: [
      "メモリ量やCPUコア数など多くの設定は、仮想マシンの電源が切れている状態でないと変更できない",
      "すべての設定は仮想マシンの起動中でなければ変更できない",
      "設定変更にはホストOSの再起動が毎回必要になる",
      "設定はVirtualBoxのインストール時に一度だけ決められ、以後は一切変更できない",
    ],
    correctChoiceIndex: 0,
    hints: ["メモリ量やCPUコア数といった基本的なハードウェア構成の変更は、稼働中には行えないことが多いです。"],
    explanation:
      "メモリ割り当て量やCPUコア数といった仮想マシンの基本的なハードウェア構成に関する設定の多くは、" +
      "仮想マシンの電源が切れている状態でのみ変更可能です。稼働中に変更したい場合は、いったんシャットダウン" +
      "してから設定画面を開く必要があります。",
  },
  {
    id: "ch01-ex35",
    chapterId: "ch01",
    type: "quiz",
    prompt: "学習中に誤ってゲストOS内の重要な設定ファイルを壊してしまった場合、VirtualBoxの機能で最も手軽に復旧できる方法はどれですか?",
    choices: [
      "作業前に取得しておいたスナップショットへ復元する",
      "ホストOSを工場出荷状態にリセットする",
      "仮想マシンを削除して、ホストOSに直接同じLinuxをインストールし直す",
      "VirtualBox自体をアンインストールしてから再インストールする",
    ],
    correctChoiceIndex: 0,
    hints: ["この章の最初の方で学んだ、まさに「失敗をなかったことにする」ための機能です。"],
    explanation:
      "作業前にスナップショットを取得しておけば、ゲストOS内で設定ファイルを壊してしまうなどの失敗をしても、" +
      "そのスナップショットへ復元するだけで手軽に元の状態へ戻せます。これが仮想マシン学習の大きな利点です。",
  },
  {
    id: "ch01-ex36",
    chapterId: "ch01",
    type: "quiz",
    prompt: "VirtualBoxで新しく仮想マシンを作成する際に、最低限決める必要がある項目として適切な組み合わせはどれですか?",
    choices: [
      "名前・OSタイプ・割り当てメモリ量・仮想ハードディスクの作成有無",
      "ホストOSのIPアドレス・DNSサーバー・デフォルトゲートウェイ",
      "ゲストOS内で使うユーザーのパスワードポリシーのみ",
      "接続するモニターの解像度とリフレッシュレートのみ",
    ],
    correctChoiceIndex: 0,
    hints: ["ネットワークの詳細設定ではなく、「どんな器を用意するか」を決める段階の話です。"],
    explanation:
      "新規仮想マシン作成のウィザードでは、名前・インストールするOSタイプ・割り当てるメモリ量・仮想ハードディスクを" +
      "新規作成するかどうか、といった基本項目を決めます。ネットワークやユーザーアカウントの詳細設定は、この後の" +
      "工程(設定画面やOSインストール中)で行います。",
  },
  {
    id: "ch01-ex37",
    chapterId: "ch01",
    type: "quiz",
    prompt: "仮想マシンの「起動順序(ブートオーダー)」設定の役割として正しいものはどれですか?",
    choices: [
      "光学ドライブ・ハードディスク・ネットワークなど、どのデバイスから優先的に起動を試みるかを決める",
      "仮想マシンを起動する曜日・時刻をスケジュールする",
      "ゲストOS内で自動起動させるアプリケーションの順番を決める",
      "複数の仮想マシンをどの順番で起動するかを決める",
    ],
    correctChoiceIndex: 0,
    hints: ["実機のBIOS/UEFI設定にある「起動順序」と同じ役割を仮想マシンに対して設定するものです。"],
    explanation:
      "起動順序(ブートオーダー)は、光学ドライブ・ハードディスク・ネットワークなど複数の起動可能なデバイスの中で、" +
      "どれを優先して起動を試みるかを決める設定です。ISOイメージからインストーラーを起動したい場合は、" +
      "光学ドライブをハードディスクより優先させる必要があります。",
  },
  {
    id: "ch01-ex38",
    chapterId: "ch01",
    type: "quiz",
    prompt: "VirtualBoxの「シームレスモード」について正しい説明はどれですか?",
    choices: [
      "ゲストOSのウィンドウの背景(デスクトップの縁)を消し、ゲストOS上のアプリのウィンドウがホストOSの" +
        "デスクトップと混在しているかのように表示するモード",
      "ゲストOSとホストOSのファイルシステムを1つに完全統合するモード",
      "複数のゲストOSを1つの画面に同時に自動配置して常に並べて表示するモード",
      "ネットワーク設定を自動的に最適なモードへ切り替える機能",
    ],
    correctChoiceIndex: 0,
    hints: ["Guest Additionsインストール後に使える、画面の見た目に関する表示モードの1つです。"],
    explanation:
      "シームレスモードは、Guest Additionsをインストール済みのゲストOSで使える表示モードの1つで、" +
      "ゲストOSのデスクトップの背景を非表示にし、ゲストOS上のアプリケーションのウィンドウをあたかもホストOSの" +
      "デスクトップ上に直接表示されているかのように見せる機能です。",
  },
  {
    id: "ch01-ex39",
    chapterId: "ch01",
    type: "quiz",
    prompt: "VirtualBoxのウィンドウとホストOSのマウス・キーボード操作の切り替えに使われる「ホストキー」の役割はどれですか?",
    choices: [
      "ゲストOSのウィンドウにマウス/キーボードの操作対象(フォーカス)が入り込んだ状態から、ホストOS側へ" +
        "操作を戻すためのキー",
      "仮想マシンを強制終了させるための唯一の手段",
      "ゲストOSにログインするためのパスワード入力用のキー",
      "スナップショットを自動的に取得するためのショートカットキー",
    ],
    correctChoiceIndex: 0,
    hints: ["マウスがゲストOSのウィンドウに「捕捉」されてしまった状態から抜け出すためのキーです。"],
    explanation:
      "Guest Additionsがない環境などでは、ゲストOSのウィンドウ内をクリックするとマウス・キーボードの操作対象が" +
      "ゲストOS側に切り替わり、そのままではホストOS側の操作に戻せなくなることがあります。ホストキー" +
      "(多くの場合、右Ctrlキー)を押すことで、操作対象をホストOS側へ戻すことができます。",
  },
  {
    id: "ch01-ex40",
    chapterId: "ch01",
    type: "quiz",
    prompt: "この教材(アプリ)で、実際のVirtualBoxのインストールや操作を行わずにCh1を学習できるようにしている理由として" +
      "最も適切なものはどれですか?",
    choices: [
      "アプリ内シミュレータは仮想ファイルシステム上でのコマンド操作を再現するものであり、VirtualBoxのインストール" +
        "作業自体はシミュレータの対象外のため、概念理解を選択式クイズで確認する形にしている",
      "VirtualBoxが有料ソフトウェアであるため",
      "VirtualBoxは現在では入手できないソフトウェアであるため",
      "Ch1の内容がLinuxコマンドと全く関係がないため",
    ],
    correctChoiceIndex: 0,
    hints: ["このアプリの採点エンジンが何を判定できるか(仮想ファイルシステム上でのコマンド実行結果)を思い出しましょう。"],
    explanation:
      "本アプリの採点エンジンは、仮想ファイルシステム(VFS)上でのコマンド実行結果を模範解答と突き合わせて判定する" +
      "仕組みです。VirtualBoxのインストールやVM作成といった実機操作はこの仕組みでは再現できないため、Ch1では" +
      "概念理解を選択式クイズで確認する形を取っています。",
  },
  ...
  },
  {
    id: "ch10-ex05",
    chapterId: "ch10",
    prompt: "自分以外のユーザーが所有するプロセスも含め、全プロセスの一覧を -A オプションで表示してください。",
    referenceSolution: "ps -A",
    processes: [
      { pid: 1001, command: "bash", status: "running", owner: "study" },
      { pid: 2001, command: "sshd", status: "running", owner: "root" },
      { pid: 2002, command: "cron", status: "running", owner: "root" },
    ],
    hints: ["-e / -a / -A はいずれも全ユーザーのプロセスを表示させるオプションです。"],
    explanation: "ps -A は ps -e と同様に、自分以外のユーザーが所有するプロセスも含めた全プロセス一覧を表示します。",
  },
  {
    id: "ch10-ex06",
    chapterId: "ch10",
    ...
    referenceSolution: "ps -a",
    processes: [
      { pid: 1001, command: "zsh", status: "running", owner: "study" },
      { pid: 1002, command: "top", status: "running", owner: "study" },
      { pid: 2001, command: "sshd", status: "running", owner: "root" },
    ],
    hints: ["-a オプションも -e / -A と同様に全ユーザーのプロセスを一覧表示させます。"],
    explanation: "ps -a も ps -e / ps -A と同じく、自分以外のユーザーが所有するプロセスも含めた全プロセス一覧を表示します。",
  },
  {
    id: "ch10-ex07",
    chapterId: "ch10",
    prompt: "PID 1001 と PID 1003 の2つのプロセスを、1回のkillコマンドで両方とも終了させてください。",
    referenceSolution: "kill 1001 1003",
    processes: [
      { pid: 1001, command: "sleep 200", status: "running", owner: "study" },
      { pid: 1002, command: "bash", status: "running", owner: "study" },
      { pid: 1003, command: "sleep 300", status: "running", owner: "study" },
    ],
    hints: ["kill は複数のPIDをスペース区切りで並べて指定すると、まとめて終了させられます。"],
    explanation:
      "kill PID1 PID2 のように複数のPIDをスペース区切りで指定すると、1回のコマンド実行でそれぞれのプロセスを" +
      "終了できます。",
  },
  {
    id: "ch10-ex08",
    chapterId: "ch10",
    prompt: "停止中のジョブが2つ(%1, %2)あります。ジョブ番号%2の方だけをバックグラウンドで再開してください。",
    referenceSolution: "bg %2",
    processes: [
      { pid: 3001, jobId: 1, command: "sleep 100 &", status: "stopped", owner: "study" },
      { pid: 3002, jobId: 2, command: "sleep 200 &", status: "stopped", owner: "study" },
    ],
    hints: ["bg %番号 のようにジョブ番号を明示すれば、複数ある停止中のジョブから特定の1つだけを再開できます。"],
    explanation:
      "停止中のジョブが複数ある場合、bg(引数なし)ではカレントジョブしか再開できません。bg %2 のようにジョブ番号を" +
      "明示することで、任意のジョブを狙って再開できます。",
  },
  {
    id: "ch10-ex09",
    chapterId: "ch10",
    prompt: "バックグラウンドで動いているジョブが2つ(%1, %2)あります。ジョブ番号%1の方をフォアグラウンドへ戻してください。",
    referenceSolution: "fg %1",
    processes: [
      { pid: 3001, jobId: 1, command: "sleep 100 &", status: "running", owner: "study" },
      { pid: 3002, jobId: 2, command: "sleep 200 &", status: "running", owner: "study" },
    ],
    hints: ["fg %番号 で、指定したジョブ番号のジョブをフォアグラウンドに呼び戻せます。"],
    explanation:
      "fg %番号 は指定したジョブ番号のジョブをフォアグラウンドに切り替えるコマンドです。複数のジョブがある場合は" +
      "番号を明示しないと、カレントジョブ(番号が最大のもの)が対象になってしまいます。",
  },
  {
    id: "ch10-ex10",
    chapterId: "ch10",
    prompt: "ジョブ番号を指定せずに fg を実行すると、カレントジョブ(番号が最大のジョブ)がフォアグラウンドに戻ります。" +
      "その動作を確認してください。",
    ...
    hints: ["ジョブ番号を省略した場合、対象になるのは jobs の出力で + が付くカレントジョブです。"],
    explanation:
      "fg をジョブ番号なしで実行すると、複数あるジョブのうち番号が最大の「カレントジョブ」(jobsの出力で+が付く" +
      "ジョブ)がフォアグラウンドに戻されます。",
  },
  {
    id: "ch10-ex11",
    ...
    processes: [
      { pid: 3001, jobId: 1, command: "sleep 100 &", status: "running", owner: "study" },
      { pid: 3002, jobId: 2, command: "vim notes.txt", status: "stopped", owner: "study" },
    ],
    hints: ["jobs コマンドは現在のシェルが管理しているジョブの一覧を表示します。"],
    explanation:
      "jobs は現在のシェルが管理しているバックグラウンド/停止中のジョブを一覧表示します。各ジョブにはジョブ番号" +
      "(%1, %2, ...)が振られ、状態(Running/Stopped)も確認できます。",
  },
  {
    id: "ch10-ex12",
    chapterId: "ch10",
    prompt: "ジョブ番号%1のプロセスを、ジョブ番号を指定してkillコマンドで終了させてください(PIDを直接指定しない方法)。",
    referenceSolution: "kill %1",
    processes: [{ pid: 3001, jobId: 1, command: "sleep 100 &", status: "running", owner: "study" }],
    hints: ["kill は PID の代わりに %番号 の形式でジョブ番号を指定して終了させることもできます。"],
    explanation:
      "kill はPIDだけでなく %番号 という形式でジョブ番号を指定してプロセスを終了させることもできます。ジョブとして" +
      "管理されているプロセスであれば、PIDを調べなくてもジョブ番号だけで終了操作が行えます。",
  },
  {
    id: "ch10-ex13",
    chapterId: "ch10",
    prompt: "PID 1002 のプロセスに対して、シグナル番号9(SIGKILL)を明示的に指定して強制終了させてください。",
    referenceSolution: "kill -9 1002",
    processes: [
      { pid: 1001, command: "bash", status: "running", owner: "study" },
      { pid: 1002, command: "sleep 100", status: "running", owner: "study" },
    ],
    hints: ["kill -9 PID のように、シグナル番号をオプションとして指定できます。"],
    explanation:
      "kill -9 PID は、シグナル番号9(SIGKILL)を明示的に指定してプロセスを終了させる書き方です。SIGKILLは" +
      "プロセス側で無視・後処理できない強制終了のシグナルで、通常のkill(SIGTERM)で終了しない場合の最終手段" +
      "として使われます。",
  },
  {
    id: "ch10-ex14",
    chapterId: "ch10",
    prompt: "root が所有するプロセス(PID 2001)を、一般ユーザーstudyの立場からkillしようとするとどうなるか確認してください。",
    referenceSolution: "kill 2001",
    processes: [
      { pid: 1001, command: "bash", status: "running", owner: "study" },
      { pid: 2001, command: "sshd", status: "running", owner: "root" },
    ],
    hints: [
      "一般ユーザーは、自分が所有していないプロセスをkillする権限を持っていません。",
      "実行するとエラーメッセージが表示されるはずです。まずは実際に試してみましょう。",
    ],
    explanation:
      "root以外のユーザーは自分が所有するプロセスしか終了させられません。root所有のプロセス(sshd等)を" +
      "一般ユーザーがkillしようとすると、Operation not permittedという権限エラーになります。これはCh9で学んだ" +
      "パーミッションの考え方がプロセス管理にも及んでいることを示しています。",
  },
  {
    id: "ch10-ex15",
    chapterId: "ch10",
    prompt: "自分(study)が所有するプロセスの一覧を表示してください(viとpingが動いている状況)。",
    referenceSolution: "ps",
    processes: [
      { pid: 1010, command: "bash", status: "running", owner: "study" },
      { pid: 1011, command: "vi report.txt", status: "running", owner: "study" },
      { pid: 1012, command: "ping example.com", status: "running", owner: "study" },
      { pid: 2010, command: "cron", status: "running", owner: "root" },
    ],
    hints: ["ps はデフォルトで自分が所有するプロセスのみを一覧表示します。"],
    explanation: "オプションなしの ps は、rootが所有するcron等を含めず、自分(study)が所有するプロセスのみを表示します。",
  },
  {
    id: "ch10-ex16",
    chapterId: "ch10",
    prompt: "PID 1005 の less コマンドのプロセスを終了させてください。",
    referenceSolution: "kill 1005",
    processes: [
      { pid: 1001, command: "bash", status: "running", owner: "study" },
      { pid: 1005, command: "less report.txt", status: "running", owner: "study" },
    ],
    hints: ["kill PID で指定したプロセスを終了できます。"],
    explanation: "less report.txt を実行しているプロセス(PID 1005)を、kill 1005 で終了させます。",
  },
  {
    id: "ch10-ex17",
    chapterId: "ch10",
    prompt: "停止中のジョブ%3をバックグラウンドで再開してください(ジョブ番号は%1, %2, %3の3つがあります)。",
    referenceSolution: "bg %3",
    processes: [
      { pid: 4001, jobId: 1, command: "sleep 100 &", status: "running", owner: "study" },
      { pid: 4002, jobId: 2, command: "sleep 200 &", status: "running", owner: "study" },
      { pid: 4003, jobId: 3, command: "sleep 300 &", status: "stopped", owner: "study" },
    ],
    hints: ["3つ以上ジョブがあっても、bg %番号 で狙ったジョブだけを個別に操作できます。"],
    explanation: "ジョブが3つ以上あっても、bg %3 のようにジョブ番号を指定すれば、狙ったジョブだけを再開できます。",
  },
  {
    id: "ch10-ex18",
    chapterId: "ch10",
    prompt: "ジョブ番号%2のプロセス(find / -name \"*.log\")をフォアグラウンドに戻してください。",
    referenceSolution: "fg %2",
    processes: [
      { pid: 4001, jobId: 1, command: "sleep 100 &", status: "running", owner: "study" },
      { pid: 4002, jobId: 2, command: 'find / -name "*.log" &', status: "running", owner: "study" },
    ],
    hints: ["fg %番号 で指定したジョブをフォアグラウンドに戻せます。"],
    explanation: "find / -name \"*.log\" のような時間のかかる処理をバックグラウンドで実行しておき、fg %2 で必要な時に" +
      "フォアグラウンドへ呼び戻す、という使い方の練習です。",
  },
  {
    id: "ch10-ex19",
    chapterId: "ch10",
    prompt: "自分以外のユーザーが所有するプロセスも含め、全プロセスの一覧を表示してください(rootの複数プロセスが" +
      "混在する状況)。",
    referenceSolution: "ps -e",
    processes: [
      { pid: 1001, command: "bash", status: "running", owner: "study" },
      { pid: 2001, command: "sshd", status: "running", owner: "root" },
      { pid: 2002, command: "systemd", status: "running", owner: "root" },
      { pid: 2003, command: "rsyslogd", status: "running", owner: "root" },
    ],
    hints: ["-e(または-a/-A)オプションを付けると全ユーザーのプロセスを表示できます。"],
    explanation: "ps -e を実行すると、study所有のbashに加えて、root所有のsshd/systemd/rsyslogdもすべて一覧に表示されます。",
  },
  {
    id: "ch10-ex20",
    chapterId: "ch10",
    prompt: "PID 5001, 5002, 5003 の3つのプロセスをまとめて終了させてください。",
    referenceSolution: "kill 5001 5002 5003",
    processes: [
      { pid: 5001, command: "sleep 100", status: "running", owner: "study" },
      { pid: 5002, command: "sleep 200", status: "running", owner: "study" },
      { pid: 5003, command: "sleep 300", status: "running", owner: "study" },
    ],
    hints: ["kill には複数のPIDをスペース区切りで指定できます。"],
    explanation: "kill PID1 PID2 PID3 のように3つ以上のPIDを並べて指定しても、まとめて終了させられます。",
  },
  {
    id: "ch10-ex21",
    chapterId: "ch10",
    prompt: "自分が所有するプロセスの一覧を確認したうえで、その中からPID 6002のプロセスだけを終了させてください。",
    referenceSolution: "ps\nkill 6002",
    processes: [
      { pid: 6001, command: "bash", status: "running", owner: "study" },
      { pid: 6002, command: "yes", status: "running", owner: "study" },
    ],
    hints: ["まず ps でPIDを確認し、そのうえで kill PID を実行する、という2段階の操作です。"],
    explanation:
      "実際の運用では、まず ps でプロセス一覧とPIDを確認してから、対象のPIDを指定して kill する、という流れが" +
      "一般的です。この演習では ps に続けて kill 6002 を実行し、一連の流れを練習します。",
  },
  {
    id: "ch10-ex22",
    chapterId: "ch10",
    prompt: "停止中のジョブ%1をバックグラウンドで再開したうえで、そのままフォアグラウンドに切り替えてください。",
    referenceSolution: "bg %1\nfg %1",
    processes: [{ pid: 7001, jobId: 1, command: "sleep 100 &", status: "stopped", owner: "study" }],
    hints: [
      "bg %1 でまずバックグラウンド実行として再開します。",
      "続けて fg %1 を実行すると、同じジョブをフォアグラウンドへ切り替えられます。",
    ],
    explanation:
      "停止中のジョブは、bg でバックグラウンド実行として再開してから、必要になった時点で fg を使って" +
      "フォアグラウンドへ切り替える、という流れで操作できます。",
  },
  {
    id: "ch10-ex23",
    chapterId: "ch10",
    prompt: "ジョブが1つもフォアグラウンド指定されていない状態で、番号を省略して bg を実行するとカレントジョブ" +
      "(唯一の停止中ジョブ)が再開されることを確認してください。",
    referenceSolution: "bg",
    processes: [{ pid: 8001, jobId: 1, command: "sleep 100 &", status: "stopped", owner: "study" }],
    hints: ["ジョブが1つしかない場合、bg も fg もジョブ番号を省略してそのジョブを対象にできます。"],
    explanation: "停止中のジョブが1つだけの場合、bg をジョブ番号なしで実行するとそのジョブがカレントジョブとして再開されます。",
  },
  {
    id: "ch10-ex24",
    chapterId: "ch10",
    prompt: "PID 9001 の tail -f access.log プロセスを終了させてください。",
    referenceSolution: "kill 9001",
    processes: [
      { pid: 1001, command: "bash", status: "running", owner: "study" },
      { pid: 9001, command: "tail -f access.log", status: "running", owner: "study" },
    ],
    hints: ["kill PID で指定したプロセスを終了できます。"],
    explanation: "tail -f のようにログを監視し続けるコマンドも、通常のプロセスと同様に kill PID で終了できます。",
  },
  {
    id: "ch10-ex25",
    chapterId: "ch10",
    type: "quiz",
    prompt: "psコマンドの出力に表示される「PID」とは何ですか?",
    choices: [
      "実行中の各プロセスに割り当てられる、一意な識別番号(プロセスID)",
      "プロセスが使用しているメモリ量(MB単位)",
      "プロセスの実行優先度を表す数値",
      "プロセスを起動したユーザーのユーザーID",
    ],
    correctChoiceIndex: 0,
    hints: ["kill コマンドで終了させたいプロセスを指定するときにも、この番号を使います。"],
    explanation:
      "PID(Process ID)は、実行中の各プロセスに割り当てられる一意な識別番号です。psで一覧を確認し、killで" +
      "特定のプロセスを終了させる際など、プロセスを個別に指定するために使われます。",
  },
  {
    id: "ch10-ex26",
    chapterId: "ch10",
    type: "quiz",
    prompt: "jobsコマンドで表示される「ジョブ番号(%1など)」とPIDの違いとして正しいものはどれですか?",
    choices: [
      "ジョブ番号は現在のシェルが管理するジョブに対してシェルが独自に振る番号で、システム全体で一意なPIDとは別物",
      "ジョブ番号とPIDは全く同じ値であり、呼び方が違うだけである",
      "ジョブ番号はシステム全体で一意だが、PIDはシェルごとに独自に振られる",
      "ジョブ番号はrootユーザーのプロセスにしか割り当てられない",
    ],
    correctChoiceIndex: 0,
    hints: ["ジョブ番号は「今のシェルの中だけ」で通用する番号、PIDは「システム全体」で通用する番号、と考えると整理できます。"],
    explanation:
      "PIDはシステム全体で一意なプロセスの識別番号ですが、ジョブ番号(%1, %2, ...)は現在の対話シェルが" +
      "バックグラウンド/停止中のジョブを管理するために独自に振る番号で、シェルごとに1から数えられます。",
  },
  {
    id: "ch10-ex27",
    chapterId: "ch10",
    type: "quiz",
    prompt: "フォアグラウンドで実行中のコマンドに対してCtrl+zを押すと、どうなりますか?",
    choices: [
      "そのコマンドの実行が一時停止し、シェルの操作に戻ってジョブとして管理できるようになる",
      "そのコマンドが即座に完全終了(kill)される",
      "そのコマンドが自動的にバックグラウンドで実行を継続する(停止はしない)",
      "シェル自体が終了してログアウトする",
    ],
    correctChoiceIndex: 0,
    hints: ["Ctrl+zは「終了」ではなく「一時停止」です。あとで fg や bg で再開できます。"],
    explanation:
      "Ctrl+zはフォアグラウンドで実行中のプロセスにSTOPシグナルを送り、実行を一時停止させます。停止したジョブは" +
      "jobsで確認でき、fgでフォアグラウンド、bgでバックグラウンドとして再開できます。",
  },
  {
    id: "ch10-ex28",
    chapterId: "ch10",
    type: "quiz",
    prompt: "bgコマンドの役割として正しいものはどれですか?",
    choices: [
      "停止中のジョブを、バックグラウンドで実行中の状態に戻す",
      "実行中のジョブを新たに停止状態にする",
      "実行中のすべてのプロセスを一覧表示する",
      "指定したプロセスを強制終了する",
    ],
    correctChoiceIndex: 0,
    hints: ["「back」+「ground」で、裏側(バックグラウンド)で動かし続けるためのコマンドです。"],
    explanation: "bgは、Ctrl+z等で停止中になっているジョブを、バックグラウンドで実行を継続する状態に戻すコマンドです。",
  },
  {
    id: "ch10-ex29",
    chapterId: "ch10",
    type: "quiz",
    prompt: "fgコマンドの役割として正しいものはどれですか?",
    choices: [
      "バックグラウンド/停止中のジョブを、フォアグラウンド(画面を占有する実行状態)に戻す",
      "実行中のジョブをバックグラウンドへ移動させる",
      "すべてのジョブを一括で終了させる",
      "現在のディレクトリのファイル一覧を表示する",
    ],
    correctChoiceIndex: 0,
    hints: ["「foreground」の略で、シェルの操作を占有する状態に戻すコマンドです。"],
    explanation: "fgは、バックグラウンドで実行中、または停止中のジョブを、フォアグラウンド(操作の主導権を持つ実行状態)に戻します。",
  },
  {
    id: "ch10-ex30",
    chapterId: "ch10",
    type: "quiz",
    prompt: "killコマンドが実際に行っていることとして最も正確な説明はどれですか?",
    choices: [
      "指定したプロセスに対してシグナルを送信する(デフォルトはSIGTERM、終了を要求するシグナル)",
      "指定したプロセスが使用しているメモリ領域を直接OSから強制的に取り上げて解放する",
      "指定したプロセスの実行ファイルをディスクから削除する",
      "指定したプロセスの優先度を下げるだけで、終了はさせない",
    ],
    correctChoiceIndex: 0,
    hints: ["名前とは裏腹に、killの正体は「プロセスへのシグナル送信」です。"],
    explanation:
      "killコマンドの本質は、指定したプロセスにシグナルを送信することです。オプションなしのkillはSIGTERM" +
      "(終了を要求するシグナル)を送り、多くのプロセスはこれを受け取ると後処理をしたうえで終了します。",
  },
  {
    id: "ch10-ex31",
    chapterId: "ch10",
    type: "quiz",
    prompt: "kill -9 PID(SIGKILL)がSIGTERM(デフォルトのkill)と異なる点として正しいものはどれですか?",
    choices: [
      "プロセス側で無視したり後処理を挟んだりすることができない、より強制力の強い終了シグナルである",
      "SIGKILLはプロセスを一時停止させるだけで、終了はさせない",
      "SIGKILLはroot以外のユーザーには一切送信できないシグナルである",
      "SIGKILLとSIGTERMは完全に同じ効果を持つ、単なる別名である",
    ],
    correctChoiceIndex: 0,
    hints: ["SIGTERMは「お願い」に近く、SIGKILLは「問答無用」の強制終了です。"],
    explanation:
      "SIGTERM(デフォルトのkill)はプロセスに終了を要求するシグナルで、プロセス側がそれを受け取って後処理をした" +
      "うえで終了できます。一方SIGKILL(kill -9)はOSがプロセスを強制的に停止させるシグナルで、プロセス側で" +
      "無視したり後処理を行ったりすることができません。SIGTERMで終了しない場合の最終手段として使われます。",
  },
  {
    id: "ch10-ex32",
    chapterId: "ch10",
    type: "quiz",
    prompt: "kill -9(SIGKILL)の多用があまり推奨されない理由として適切なものはどれですか?",
    choices: [
      "プロセスが後処理(一時ファイルの削除やデータの保存等)を行う機会を与えずに強制終了させてしまうため",
      "SIGKILLはシステムを完全にシャットダウンしてしまうため",
      "SIGKILLは1日に1回しか使用できない制限があるため",
      "kill -9はrootユーザーのプロセスにしか効果がないため",
    ],
    correctChoiceIndex: 0,
    hints: ["「後片付けをする時間を与えない」終了のさせ方であることを思い出しましょう。"],
    explanation:
      "SIGKILLはプロセスに後処理の機会を一切与えずに強制終了させるため、一時ファイルの削除やデータの保存などの" +
      "終了処理が行われないまま止まってしまうことがあります。まずは通常のkill(SIGTERM)を試し、それでも終了" +
      "しない場合の最終手段としてkill -9を使うのが基本です。",
  },
  {
    id: "ch10-ex33",
    chapterId: "ch10",
    ...
    choices: [
      "そのコマンドがバックグラウンドで実行され、シェルはすぐに次の入力を受け付けられるようになる",
      "そのコマンドが2回連続で実行される",
      "そのコマンドの標準出力がファイルに保存される",
      "そのコマンドの実行がすぐに一時停止される",
    ],
    correctChoiceIndex: 0,
    hints: ["Ctrl+zで後から止めるのとは違い、最初からバックグラウンドで実行を開始する書き方です。"],
    explanation:
      "コマンドの末尾に & を付けると、そのコマンドは最初からバックグラウンドで実行され、シェルは実行完了を待たずに" +
      "すぐ次のコマンド入力を受け付けられるようになります。",
  },
  {
    id: "ch10-ex34",
    chapterId: "ch10",
    type: "quiz",
    prompt: "jobsコマンドの出力で、あるジョブの行に + が付いている場合、それは何を表していますか?",
    choices: [
      "そのジョブが「カレントジョブ」であり、fgやbgを引数なしで実行した際のデフォルトの対象になることを表す",
      "そのジョブがrootユーザーによって起動されたことを表す",
      "そのジョブが異常終了(エラー)したことを表す",
      "そのジョブが優先度の高い「リアルタイムプロセス」であることを表す",
    ],
    correctChoiceIndex: 0,
    hints: ["「+」が付いたジョブこそ、番号を省略したfg/bgが対象にするジョブです。"],
    explanation:
      "jobsの出力で + が付くジョブは「カレントジョブ」で、ジョブ番号を省略してfgやbgを実行した際にデフォルトで" +
      "対象となるジョブです。通常は最も新しく操作対象になったジョブが+になります。",
  },
  {
    id: "ch10-ex35",
    chapterId: "ch10",
    ...
    prompt: "jobsコマンドの出力で、あるジョブの行に - が付いている場合、それは何を表していますか?",
    choices: [
      "カレントジョブ(+)の次に新しい、「1つ前のジョブ」であることを表す",
      "そのジョブがすでに完全に終了していることを表す",
      "そのジョブの実行がエラーで失敗したことを表す",
      "そのジョブがバックグラウンドで実行できないことを表す",
    ],
    correctChoiceIndex: 0,
    hints: ["+ が「カレントジョブ」なのに対し、- は「1つ前のジョブ」を表すマークです。"],
    explanation: "jobsの出力で - が付くジョブは、カレントジョブ(+)の次に新しい「1つ前のジョブ」であることを表します。",
  },
  {
    id: "ch10-ex36",
    chapterId: "ch10",
    type: "quiz",
    prompt: "psコマンドをオプションなしで実行した場合、デフォルトで表示される範囲はどれですか?",
    choices: [
      "自分(実行したユーザー)が所有するプロセスのみ",
      "システム上のすべてのユーザーのすべてのプロセス",
      "root所有のプロセスのみ",
      "バックグラウンドジョブとして登録されているプロセスのみ",
    ],
    correctChoiceIndex: 0,
    hints: ["全ユーザー分を見たい場合は -e / -a / -A オプションが必要でした。"],
    explanation:
      "psをオプションなしで実行すると、デフォルトでは自分(実行したユーザー)が所有するプロセスのみが表示されます。" +
      "他のユーザーが所有するプロセスも含めて見たい場合は -e(または -a / -A)を付けます。",
  },
  {
    id: "ch10-ex37",
    chapterId: "ch10",
    type: "quiz",
    prompt: "一般ユーザーがroot所有のプロセスをkillしようとすると、どうなりますか?",
    choices: [
      "Operation not permitted のような権限エラーになり、終了させられない",
      "問題なく終了させることができる",
      "そのユーザーのアカウントが自動的にロックされる",
      "root所有のプロセスがそのユーザーの所有に自動的に変わる",
    ],
    correctChoiceIndex: 0,
    hints: ["ファイルのパーミッションと同じように、プロセスの操作にも所有者に基づく権限の考え方があります。"],
    explanation:
      "root以外の一般ユーザーは、自分が所有していないプロセス(root所有のプロセス等)をkillする権限を持っていません。" +
      "実行しようとするとOperation not permittedのような権限エラーになります。",
  },
  {
    id: "ch10-ex38",
    chapterId: "ch10",
    type: "quiz",
    prompt: "ps の出力のSTAT(状態)列に表示される「R」と「T」は、それぞれ何を表しますか?",
    choices: [
      "R = 実行中(Running)、T = 停止中(Stopped)",
      "R = 削除済み(Removed)、T = 一時ファイル(Temporary)",
      "R = 読み取り専用(Read-only)、T = 書き込み可能(writable)",
      "R = root所有、T = 一般ユーザー(Third-party)所有",
    ],
    correctChoiceIndex: 0,
    hints: ["ps一覧のプロセスは大きく「実行中」か「停止中」のいずれかの状態を持ちます。"],
    explanation: "psの状態(STAT)列のRはRunning(実行中)、TはStopped/Traced相当(停止中)を表します。",
  },
  {
    id: "ch10-ex39",
    chapterId: "ch10",
    type: "quiz",
    prompt: "同じキーワードを含むコマンドを繰り返し実行しても壊れないよう、jobsやpsの状態確認を挟んでから操作する" +
      "手順が推奨される理由として適切なものはどれですか?",
    choices: [
      "対象のPID・ジョブ番号を正しく特定してから操作することで、意図しないプロセスを誤って終了させる事故を防げるため",
      "jobsやpsを実行しないと、bashそのものが起動しなくなるため",
      "確認を挟むと、プロセスの実行速度が自動的に速くなるため",
      "確認を挟まないと、killコマンド自体が使用できない仕様になっているため",
    ],
    correctChoiceIndex: 0,
    hints: ["「先に確認、それから操作」という手順は、事故防止のための基本的な考え方です。"],
    explanation:
      "psやjobsで現在の状況(PIDやジョブ番号)を正しく確認してから fg/bg/kill を実行することで、番号の思い違いに" +
      "よって意図しない別のプロセスを操作・終了してしまう事故を防げます。",
  },
  {
    id: "ch10-ex40",
    chapterId: "ch10",
    type: "quiz",
    prompt: "フォアグラウンドとバックグラウンドの違いとして正しいものはどれですか?",
    choices: [
      "フォアグラウンドは端末の入出力を占有して実行される状態、バックグラウンドは端末の入力を占有せず裏側で" +
        "実行され続ける状態",
      "フォアグラウンドは常にroot権限、バックグラウンドは常に一般ユーザー権限で実行される",
      "フォアグラウンドは終了できないが、バックグラウンドはいつでも終了できる",
      "フォアグラウンドとバックグラウンドは処理速度が異なるだけで、それ以外の違いはない",
    ],
    correctChoiceIndex: 0,
    hints: ["フォアグラウンド実行中は、そのコマンドが終わるまで次のコマンドを入力できません。"],
    explanation:
      "フォアグラウンドで実行中のコマンドは端末の入出力を占有し、終了するまで次のコマンドを入力できません。" +
      "一方バックグラウンドで実行中のコマンドは端末の入力を占有せず、裏側で処理を続けながら並行して次の" +
      "コマンドを入力できます。",
  },
